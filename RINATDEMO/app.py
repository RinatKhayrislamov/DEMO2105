...
def create_app():
    ...
    with app.app_context():
        ...
        try:
            ...
            if not admin_user:
                ...
                if admin_user.username != 'Admin26':
                    admin_user.username = 'Admin26'
                admin_user.password = generate_password_hash('Demo20')
                admin_user.is_admin = True
                admin_user.status = 'Активен'
                db.session.commit()
                print("✅ Данные администратора обновлены (login: Admin26, pass: Demo20)")
    ...
    def add_review(req_id):
        ...
        # Ручная обработка формы для отзыва, потомучто у нас модальное окно
        ...
        if rating and text:
            try:
                ...
                db.session.rollback()
                flash('Ошибка при сохранении отзыва. Возможно, он уже существует.', 'danger')
